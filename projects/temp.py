...
# data = pd.read_csv('/Users/jing/pydata/datasets/plantiq/广西卷烟厂/m114_m_all.csv')
# data = data[['重量', '圆周长', '长度', '硬度']]
data = pd.read_csv('/Users/jing/pydata/datasets/plantiq/广西卷烟厂/m114_p.csv')
...
# #     data.iloc[:, i] = data.iloc[:, i].where(abs(data.iloc[:, i] - data.iloc[:, i].mean()) <= data.iloc[:, i].std() * 2, data.iloc[:, i].mean())
# #分位数法
...
#
# for i in range(num_col):
#     data.iloc[:, i] = data.iloc[:, i].where((data.iloc[:, i] <= upcl[i]) & (data.iloc[:, i] >= lowcl[i]), data.iloc[:, i].mean())

# Xtrain = np.array(data[:3000])
# Xtest = np.array(data[3000:])
...
for i in range(n):
    ...
    Q[i] = Xtest[i,:].dot((I-P.dot(P.T))).dot(Xtest[i,:].T)
